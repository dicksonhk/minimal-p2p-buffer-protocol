...
1.  **Liveness Check (PING/PONG Operation):**
    ...
    -   The receiving party _must_ respond with a `PONG` packet, following specific rules based on the PING received (see "PING/PONG Behavior" below).
...
### General Expected Behaviors (Client & Axon Relay)
...
-   **Handling of Empty Message Data:** All parties SHOULD ignore `MSG`, `GET_MSG_ACK`, or `PUT_MSG` packets where the `data` field is empty, unless explicitly meaningful for the application.
-   **Idempotency:**
...
-   **Acknowledgments (`MSG_ACK`, `PUT_MSG_ACK`, `GET_MSG_ACK`, `LIST_MSG_ACK`):**
...
### Axon Relay Storage: Partitioning for Scalability
...
## Deployment and Operational Considerations (for Axon Relays)
...
### 2. DNS-Level Routing and Operational Benefits (for Axon Relay Services)
...
### 3. Managing Old Versions (of Axon Relay Deployments)
