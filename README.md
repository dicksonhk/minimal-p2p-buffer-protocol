...
### 4.4. General Expected Behaviors (Client & Axon Relay)
   ...
   4.4.10. **Strict Adherence to Standard Format:** For standard packet types (MSB of `PacketType` is 0, or `PacketType` is `0xFF`), Clients and Axon Relays MUST strictly follow the documented wire format.
...
A critical design aspect for achieving scalability with a massive number of concurrent Channels is the **partitioning of storage on a per-tenant or per-Channel basis** by the Axon Relay component.
   4.6.1. **Why Partitioning is Crucial:** Isolation, scalability, resource management, data lifecycle, concurrency.
   ...
   4.6.3. **Schema within a Partition:**
      ```sql
      ...
      ```
   4.6.4. **Note on Non-Persistent Messages:** Messages sent via `DIRECT_SEND` or `FAST_SEND` (effectively TTL `0`, relayed with `message_id = 0`) are not intended for persistent storage within this backend by the Axon Relay.
...
### 4.8. Out-of-Scope for this Core Protocol
   ...
   4.8.1. **Explicit Channel Lifecycle Management APIs.**
   4.8.2. **Axon Relay Statistics and Monitoring.**

### 4.9. General System Design Notes

   4.9.1. **At-Least-Once Delivery (for Buffered Messages):** Core goal for `PUT_MSG`.
   4.9.2. **Error Handling:** Recoverable errors via `NACK`. Terminating conditions via specific `NACK`s.
   4.9.3. **Security:** TLS for transport; application-level E2E encryption for payload recommended.
   4.9.4. **Client Discovery & Channel Initiation:** External mechanisms.
   4.9.5. **Snowflake IDs:** Recommended for `message_id` for buffered messages (non-zero).
